# Claude Code Go

A Go implementation of [Claude Code](https://github.com/anthropics/claude-code).

> **Note**: This is an unofficial community-driven implementation.

## Documentation

- [Architecture Overview](docs/architecture.md) — 系统架构概览
- [Agent Loop](docs/agent-loop.md) — 核心对话循环
- [Context & Prompt](docs/context-and-prompt.md) — 上下文与提示词管理
- [Memory System](docs/memory-system.md) — 多层级记忆系统
- [Tool System](docs/tool-system.md) — 工具系统设计
- [Task & Multi-Agent](docs/task-and-multi-agent.md) — 任务与多 Agent 协作
- [Skills & Permissions](docs/skill-and-permissions.md) — 技能与权限系统

## Status

🚧 Work in Progress

## License

MIT
